...
bool SURFMLP::Classify(float* score, float* outputs) {
    ...
    if (score != nullptr) *score = output_buf_[0];
    if (outputs != nullptr) {
...
void SURFMLP::AddFeatureByID(int32_t feat_id) { feat_id_.push_back(feat_id); }

void SURFMLP::AddLayer(int32_t input_dim, int32_t output_dim,
                       const float* weights, const float* bias,
                       bool is_output) {
    if (model_->GetLayerNum() == 0) input_buf_.resize(input_dim);
