...
#pragma omp for nowait
        for (int32_t i = 0; i < output_dim_; i++) {
            output[i] =
                seeta::fd::MathFunction::VectorInnerProduct(
                    input, weights_.data() + i * input_dim_, input_dim_) +
                bias_[i];
            output[i] =
                (act_func_type_ == 1 ? ReLU(output[i]) : Sigmoid(-output[i]));
        }
    }
}
...
void MLP::Compute(const float* input, float* output) {
    ...
    for (i = 1; i < layers_.size() - 1; i++) {
        ...
        layers_[i]->Compute(layer_buf_[(i + 1) % 2].data(),
                            layer_buf_[i % 2].data());
    }
...
void MLP::AddLayer(int32_t inputDim, int32_t outputDim, const float* weights,
    ...
    std::shared_ptr<seeta::fd::MLPLayer> layer(
        new seeta::fd::MLPLayer(is_output ? 0 : 1));
