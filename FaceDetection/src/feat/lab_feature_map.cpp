...
float LABFeatureMap::GetStdDev() const {
    ...
    if (roi_.x != 0) {
        if (roi_.y != 0) {
            ...
            m2 =
                (square_int_img_[bottom_right] - square_int_img_[bottom_left]) /
                area;
        }
    } else {
        if (roi_.y != 0) {
...
void LABFeatureMap::ComputeIntegralImages(const uint8_t* input) {
    ...
    seeta::fd::MathFunction::Square(int_img_.data(), square_int_img_.data(),
                                    len);
...
void LABFeatureMap::ComputeRectSum() {
    ...
    *rect_sum = *(int_img + (rect_height_ - 1) * width_ + rect_width_ - 1);
    seeta::fd::MathFunction::VectorSub(
        int_img + (rect_height_ - 1) * width_ + rect_width_,
        int_img + (rect_height_ - 1) * width_, rect_sum + 1, width);
...
#pragma omp for nowait
        for (int32_t i = 1; i <= height; i++) {
            ...
            *(dest++) = (*bottom_right) - (*top_right);
            seeta::fd::MathFunction::VectorSub(bottom_right + 1, top_right + 1,
                                               dest, width);
        ...
        for (int32_t r = 0; r <= height; r++) {
            for (int32_t c = 0; c <= width; c++) {
                ...
                *dest = 0;
                ...
                int32_t white_rect_sum =
                    rect_sum_[(r + rect_height_) * width_ + c + rect_width_];
                int32_t black_rect_idx = r * width_ + c;
                *dest |=
                    (white_rect_sum >= rect_sum_[black_rect_idx] ? 0x80 : 0x0);
                black_rect_idx += rect_width_;
                *dest |=
                    (white_rect_sum >= rect_sum_[black_rect_idx] ? 0x40 : 0x0);
                black_rect_idx += rect_width_;
                *dest |=
                    (white_rect_sum >= rect_sum_[black_rect_idx] ? 0x20 : 0x0);
                black_rect_idx += offset;
                *dest |=
                    (white_rect_sum >= rect_sum_[black_rect_idx] ? 0x08 : 0x0);
                black_rect_idx += offset;
                *dest |=
                    (white_rect_sum >= rect_sum_[black_rect_idx] ? 0x01 : 0x0);
                black_rect_idx -= rect_width_;
                *dest |=
                    (white_rect_sum >= rect_sum_[black_rect_idx] ? 0x02 : 0x0);
                black_rect_idx -= rect_width_;
                *dest |=
                    (white_rect_sum >= rect_sum_[black_rect_idx] ? 0x04 : 0x0);
                black_rect_idx -= offset;
                *dest |=
                    (white_rect_sum >= rect_sum_[black_rect_idx] ? 0x10 : 0x0);
            }
        }
    }
}
