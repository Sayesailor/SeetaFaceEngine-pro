...
void SURFFeaturePool::Create() {
    if (sample_height_ - patch_min_height_ <=
        sample_width_ - patch_min_width_) {
...
void SURFFeaturePool::AddAllFeaturesToPool(int32_t width, int32_t height,
    ...
    for (int32_t y = 0; y <= sample_height_ - height; y += patch_move_step_y_) {
        feat.patch.y = y;
        for (int32_t x = 0; x <= sample_width_ - width;
             x += patch_move_step_x_) {
            feat.patch.x = x;
...
void SURFFeatureMap::GetFeatureVector(int32_t feat_id, float* feat_vec) {
    if (buf_valid_[feat_id] == 0) {
        ComputeFeatureVector(feat_pool_[feat_id],
                             feat_vec_buf_[feat_id].data());
        NormalizeFeatureVectorL2(
            feat_vec_buf_[feat_id].data(), feat_vec_normed_buf_[feat_id].data(),
...
#pragma omp for nowait
        for (int32_t r = 1; r < height_ - 1; r++) {
            ...
            seeta::fd::MathFunction::VectorSub(src + (width_ << 1), src, dest,
                                               len);
        }
    }
...
void SURFFeatureMap::ComputeFeatureVector(const SURFFeature& feat,
    ...
    int32_t cell_width =
        feat.patch.width / feat.num_cell_per_row * kNumIntChannel;
    ...
    if (init_cell_y != 0) {
        if (init_cell_x != 0) {
            ...
            for (int32_t i = 0; i < kNumIntChannel; i++) {
                ...
                cell_bottom_left[i] =
                    cell_top_left[i] + row_width * cell_height;
            ...
            // cell #1
            offset =
                row_width * (init_cell_y - 1) + cell_width - kNumIntChannel;
            for (int32_t i = 0; i < kNumIntChannel; i++) {
                cell_top_right[i] = int_img + (offset++);
                cell_bottom_right[i] =
                    cell_top_right[i] + row_width * cell_height;
                tmp_cell_top_right[i] = cell_bottom_right[i];
        ...
        if (init_cell_x != 0) {
            ...
            for (int32_t i = 0; i < kNumIntChannel; i++) {
                ...
                cell_bottom_right[i] = cell_bottom_left[i] + cell_width;
                *(feat_val++) =
                    *(cell_bottom_right[i]) - *(cell_bottom_left[i]);
            ...
            for (int32_t i = 1; i < feat.num_cell_per_row; i++) {
                for (int32_t j = 0; j < kNumIntChannel; j++) {
                    ...
                    cell_bottom_right[j] += cell_width;
                    *(feat_val++) =
                        *(cell_bottom_right[j]) - *(cell_bottom_left[j]);
                }
            }
        } else {
            // cell #1
            offset =
                row_width * (cell_height - 1) + cell_width - kNumIntChannel;
            for (int32_t i = 0; i < kNumIntChannel; i++) {
    ...
    offset = cell_height * row_width - feat.patch.width * kNumIntChannel +
             cell_width;
...
void SURFFeatureMap::NormalizeFeatureVectorL2(const int32_t* feat_vec,
    ...
    if (prod != 0) {
        ...
        for (int32_t i = 0; i < len; i++) feat_vec_normed[i] = 0.0f;
    }
}
