...
bool FuStDetector::LoadModel(const std::string& model_path) {
    ...
    if (!model_file.is_open()) {
        ...
        model_file.read(reinterpret_cast<char*>(&num_hierarchy_),
                        sizeof(int32_t));
        for (int32_t i = 0; is_loaded && i < num_hierarchy_; i++) {
            ...
            for (int32_t j = 0; is_loaded && j < hierarchy_size; j++) {
                model_file.read(reinterpret_cast<char*>(&num_stage),
                                sizeof(int32_t));
                num_stage_.push_back(num_stage);

                for (int32_t k = 0; is_loaded && k < num_stage; k++) {
                    model_file.read(reinterpret_cast<char*>(&type_id),
                                    sizeof(int32_t));
                    classifier_type =
                        static_cast<seeta::fd::ClassifierType>(type_id);
                    ...
                    if (is_loaded) {
                        ...
                        if (cls2feat_idx_.count(classifier_type) == 0) {
                            feat_map_.push_back(
                                CreateFeatureMap(classifier_type));
                            cls2feat_idx_.insert(std::map<
                                seeta::fd::ClassifierType,
                                int32_t>::value_type(classifier_type,
                                                     feat_map_index++));
                        }
                ...
                model_file.read(reinterpret_cast<char*>(&num_wnd_src),
                                sizeof(int32_t));
                if (num_wnd_src > 0) {
                    wnd_src_id_.back().resize(num_wnd_src);
                    for (int32_t k = 0; k < num_wnd_src; k++) {
                        model_file.read(
                            reinterpret_cast<char*>(&(wnd_src_id_.back()[k])),
                            sizeof(int32_t));
                    }
                }
            }
...
std::vector<seeta::FaceInfo> FuStDetector::Detect(
    ...
    std::vector<std::vector<seeta::FaceInfo> > proposals_nms(
        hierarchy_size_[0]);
    for (int32_t i = 0; i < hierarchy_size_[0]; i++) {
        seeta::fd::NonMaximumSuppression(&(proposals[i]), &(proposals_nms[i]),
                                         0.8f);
        proposals[i].clear();
    }
    ...
    for (int32_t i = 1; i < num_hierarchy_; i++) {
        ...
        for (int32_t j = 0; j < hierarchy_size_[i]; j++) {
            int32_t num_wnd_src =
                static_cast<int32_t>(wnd_src_id_[cls_idx].size());
            ...
            for (int32_t k = 0; k < num_wnd_src; k++) {
                proposals[buf_idx[j]].insert(proposals[buf_idx[j]].end(),
                                             proposals_nms[wnd_src[k]].begin(),
                                             proposals_nms[wnd_src[k]].end());
            }
            ...
            for (int32_t k = 0; k < num_stage_[cls_idx]; k++) {
                int32_t num_wnd =
                    static_cast<int32_t>(proposals[buf_idx[j]].size());
                ...
                for (int32_t m = 0; m < num_wnd; m++) {
                    ...
                    if (model_[model_idx]->Classify(&score,
                                                    mlp_predicts.data())) {
                        ...
                        bboxes[bbox_idx].bbox.width = static_cast<int32_t>(
                            (mlp_predicts[3] * 2 - 1) * w + w + 0.5);
                        bboxes[bbox_idx].bbox.height =
                            bboxes[bbox_idx].bbox.width;
                        bboxes[bbox_idx].bbox.x = static_cast<int32_t>(
                            (mlp_predicts[1] * 2 - 1) * w + x +
                            (w - bboxes[bbox_idx].bbox.width) * 0.5 + 0.5);
                        bboxes[bbox_idx].bbox.y = static_cast<int32_t>(
                            (mlp_predicts[2] * 2 - 1) * h + y +
                ...
                if (k < num_stage_[cls_idx] - 1) {
                    seeta::fd::NonMaximumSuppression(
...
std::shared_ptr<seeta::fd::ModelReader> FuStDetector::CreateModelReader(
    seeta::fd::ClassifierType type) {
    std::shared_ptr<seeta::fd::ModelReader> reader;
    switch (type) {
...
std::shared_ptr<seeta::fd::Classifier> FuStDetector::CreateClassifier(
    seeta::fd::ClassifierType type) {
    std::shared_ptr<seeta::fd::Classifier> classifier;
    switch (type) {
        case seeta::fd::ClassifierType::LAB_Boosted_Classifier:
...
std::shared_ptr<seeta::fd::FeatureMap> FuStDetector::CreateFeatureMap(
    seeta::fd::ClassifierType type) {
    std::shared_ptr<seeta::fd::FeatureMap> feat_map;
    switch (type) {
        case seeta::fd::ClassifierType::LAB_Boosted_Classifier:
...
void FuStDetector::GetWindowData(const seeta::ImageData& img,
    ...
    if (pad_bottom > 0) std::memset(dest, 0, len * pad_bottom);
