...
void NonMaximumSuppression(std::vector<seeta::FaceInfo>* bboxes,
                           std::vector<seeta::FaceInfo>* bboxes_nms,
                           float iou_thresh) {
    bboxes_nms->clear();
    ...
    while (!all_merged) {
        ...
        float area1 =
            static_cast<float>(select_bbox.width * select_bbox.height);
        ...
        for (int32_t i = select_idx; i < num_bbox; i++) {
            ...
            float w = std::min<float>(
                          x2, static_cast<float>(bbox_i.x + bbox_i.width - 1)) -
                      x + 1;
            float h =
                std::min<float>(
                    y2, static_cast<float>(bbox_i.y + bbox_i.height - 1)) -
                y + 1;
            if (w <= 0 || h <= 0) continue;
