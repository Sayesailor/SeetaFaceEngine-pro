...
class MathFunction {
   public:
    ...
    static inline void VectorAbs(const int32_t* src, int32_t* dest,
                                 int32_t len) {
        int32_t i;
#ifdef USE_SSE
        __m128i val;
        ...
        for (; i < len; i++) dest[i] = (src[i] >= 0 ? src[i] : -src[i]);
#else
        for (i = 0; i < len; i++) dest[i] = (src[i] >= 0 ? src[i] : -src[i]);
#endif
    }
    ...
    static inline void Square(const int32_t* src, uint32_t* dest, int32_t len) {
        ...
        for (; i < len; i++) *(dest + i) = (*(src + i)) * (*(src + i));
#else
        for (i = 0; i < len; i++) *(dest + i) = (*(src + i)) * (*(src + i));
#endif
    }
    ...
    static inline float VectorInnerProduct(const float* x, const float* y,
        ...
        for (; i < len; i++) prod += x[i] * y[i];
#else
        for (i = 0; i < len; i++) prod += x[i] * y[i];
#endif
        return prod;
    }
};
