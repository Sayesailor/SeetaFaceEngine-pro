 ...
 *China.
 *
 * The codes are mainly developed by Jie Zhang (a Ph.D supervised by Prof.
 *Shiguang Shan)
 *
...
/** Detect five facial landmarks, i.e., two eye centers, nose tip and two mouth
 * corners.
  ...
  */
void CCFAN::FacialPointLocate(const unsigned char *gray_im, int im_width,
                              int im_height, seeta::FaceInfo face_loc,
                              float *facial_loc) {
    ...
    int extend_lx =
        std::max(int(floor(left_x - extend_factor * bbox_w)), int(0));
    int extend_rx = std::min(int(floor(right_x + extend_factor * bbox_w)),
                             int(im_width - 1));
    int extend_ly = std::max(
        int(floor(left_y - (extend_factor - extend_revised_y) * bbox_h)),
        int(0));
    int extend_ry = std::min(
        int(floor(right_y + (extend_factor + extend_revised_y) * bbox_h)),
        int(im_height - 1));
    ...
    for (int h = 0; h < face_h; h++) {
        const unsigned char *p_origin =
            gray_im + (h + extend_ly) * im_width + extend_lx;
    ...
    ResizeImage(face_patch, face_w, face_h, lan1_patch, lan1_resize_w,
                lan1_resize_h);
    ...
    for (int i = 0; i < lan1_size_ - 1; i++) {
        for (int j = 0; j < lan1_structure_[i + 1]; j++) {
            ...
            for (int k = 0; k < fea_dim; k++) {
                inner_product =
                    inner_product + lan1_a[i][k] * lan1_w_[i][j * fea_dim + k];
            }
            if (i == lan1_size_ - 2) {
                lan1_a[i + 1][j] = inner_product + lan1_b_[i][j];
            } else {
                lan1_a[i + 1][j] =
                    1.0 / (1 + exp(-inner_product - lan1_b_[i][j]));
            }
        }
    }
    ...
    ResizeImage(face_patch, face_w, face_h, lan2_patch, lan2_resize_w,
                lan2_resize_h);
    ...
    for (int i = 0; i < lan2_size_ - 1; i++) {
        for (int j = 0; j < lan2_structure_[i + 1]; j++) {
            ...
            for (int k = 0; k < fea_dim; k++) {
                inner_product =
                    inner_product + lan2_a[i][k] * lan2_w_[i][j * fea_dim + k];
            }
            if (i == lan2_size_ - 2) {
                lan2_a[i + 1][j] = inner_product + lan2_b_[i][j];
            } else {
                lan2_a[i + 1][j] =
                    1.0 / (1 + exp(-inner_product - lan2_b_[i][j]));
            }
        }
    }
    for (int i = 0; i < pts_num_ * 2; i++) {
...
/** Extract shape indexed SIFT features.
  ...
  *  @param[out] sift_fea the extracted shape indexed SIFT features which are
 * concatenated into a vector
  */
void CCFAN::TtSift(const unsigned char *gray_im, int im_width, int im_height,
                   float *face_shape, int patch_size, double *sift_fea) {
    ...
    for (int i = 0; i < pts_num_; i++) {
        /*Get one image patch*/
        GetSubImg(gray_im, im_width, im_height, face_shape[i * 2],
                  face_shape[i * 2 + 1], patch_size, sub_img);
...
/** Extract a image patch which is centered at point(point_x, point_y) with a
 * given patch size.
  *  @param gray_im A grayscale image
  *  @param im_width The width of the inpute image
  *  @param im_height The height of the inpute image
  ...
  */
void CCFAN::GetSubImg(const unsigned char *gray_im, int im_width, int im_height,
                      float point_x, float point_y, int patch_size,
                      BYTE *sub_img) {
    ...
    int patch_right =
        std::min((center_x + 1) + patch_size / 2 - 1, im_width - 1);
    int patch_top = std::max((center_y + 1) - patch_size / 2, 0);
    int patch_bottom =
        std::min((center_y + 1) + patch_size / 2 - 1, im_height - 1);

    int lx = abs(patch_left - ((center_x + 1) - patch_size / 2));
    int rx = patch_size -
             abs(patch_right - ((center_x + 1) + patch_size / 2 - 1)) - 1;
    int ty = abs(patch_top - ((center_y + 1) - patch_size / 2));
    int by = patch_size -
             abs(patch_bottom - ((center_y + 1) + patch_size / 2 - 1)) - 1;
...
/** Resize the image by bilinear interpolation.
  ...
  */
bool CCFAN::ResizeImage(const unsigned char *src_im, int src_width,
                        int src_height, unsigned char *dst_im, int dst_width,
                        int dst_height) {
    ...
    for (int n_y_d = 0; n_y_d < dst_height; n_y_d++) {
        for (int n_x_d = 0; n_x_d < dst_width; n_x_d++) {
            ...
            double lf_new_gray =
                (1 - lf_weight_y) *
                    ((1 - lf_weight_x) * src_im[n_y_s * src_width + n_x_s] +
                     lf_weight_x * src_im[n_y_s * src_width + n_x_s + 1]) +
                lf_weight_y *
                    ((1 - lf_weight_x) *
                         src_im[(n_y_s + 1) * src_width + n_x_s] +
