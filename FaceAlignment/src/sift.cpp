 ...
 *
 * This file is part of the SeetaFace Alignment module, containing codes
 *implementing the
 ...
 *China.
 *
 * The codes are firstly developed by Mengyi Liu (a Ph.D supervised by Prof.
 *Shiguang Shan) and
 * further improved by Jie Zhang (a Ph.D supervised by Prof. Shiguang Shan) for
 *more efficiency.
 *
...
double SIFT::delta_gauss_x[25] = {
    0.0284161904936934,  0.0260724940559495,  0,
    -0.0260724940559495, -0.0284161904936934, 0.127352530356230,
    0.116848811647003,   0,                   -0.116848811647003,
    -0.127352530356230,  0.209968825675801,   0.192651121218447,
    0,                   -0.192651121218447,  -0.209968825675801,
    0.127352530356230,   0.116848811647003,   0,
    -0.116848811647003,  -0.127352530356230,  0.0284161904936934,
    0.0260724940559495,  0,                   -0.0260724940559495,
    -0.0284161904936934};

double SIFT::delta_gauss_y[25] = {
    0.0284161904936934,  0.127352530356230,   0.209968825675801,
    0.127352530356230,   0.0284161904936934,  0.0260724940559495,
    0.116848811647003,   0.192651121218447,   0.116848811647003,
    0.0260724940559495,  0,                   0,
    0,                   0,                   0,
    -0.0260724940559495, -0.116848811647003,  -0.192651121218447,
    -0.116848811647003,  -0.0260724940559495, -0.0284161904936934,
    -0.127352530356230,  -0.209968825675801,  -0.127352530356230,
    -0.0284161904936934};
 ...
 */
void SIFT::InitSIFT(int im_width, int im_height, int patch_size,
                    int grid_spacing) {
    ...
    param.patch_cnt_width =
        (param.image_width - param.patch_size) / param.grid_spacing + 1;
    param.patch_cnt_height =
        (param.image_height - param.patch_size) / param.grid_spacing + 1;
    param.patch_dims = param.sample_nums * param.angle_nums;
    param.image_dims =
        param.patch_cnt_width * param.patch_cnt_height * param.patch_dims;
 ...
 */
void SIFT::filter2(double* gray_im, double* kernel, int kernel_size,
                   double* filter_im) {
    // Padding the image
    int pad_size = (kernel_size - 1) / 2;
    double* gray_img_ex = new double[(param.image_width + (kernel_size - 1)) *
                                     (param.image_height + (kernel_size - 1))];
    ...
    for (int i = param.image_height + pad_size;
         i < param.image_height + (kernel_size - 1); i++) {
        for (int j = 0; j < param.image_width + (kernel_size - 1); j++) {
            gray_img_ex[i * (param.image_width + (kernel_size - 1)) + j] = 0;
        }
    }
    ...
    for (int i = pad_size; i < param.image_height + pad_size; i++) {
        ...
        for (int j = param.image_width + pad_size;
             j < param.image_width + (kernel_size - 1); j++) {
            gray_img_ex[i * (param.image_width + (kernel_size - 1)) + j] = 0;
        }
        for (int j = pad_size; j < param.image_width + pad_size; j++) {
            gray_img_ex[i * (param.image_width + (kernel_size - 1)) + j] =
                gray_im[(i - pad_size) * param.image_width + (j - pad_size)];
        }
    }
        ...
        for (int j = 0; j < param.image_width; j++) {
            ...
            for (int ki = 0; ki < kernel_size; ki++) {
                for (int kj = 0; kj < kernel_size; kj++) {
                    double tmp1 = gray_img_ex[(i + ki) * (param.image_width +
                                                          (kernel_size - 1)) +
                                              (j + kj)];
 ...
 */
void SIFT::SparseFilter2(double* gray_im, double* kernel, int kernel_size,
                         double* filter_im) {
    // Padding the image
    int pad_size = (kernel_size - 1) / 2;
    double* gray_img_ex = new double[(param.image_width + (kernel_size - 1)) *
...
void SIFT::ConvImage(double* image_orientation, double* conv_im) {
    ...
    for (int k = 0; k < param.patch_size; k++) {
        weight[k] =
            abs(k - double(param.patch_size - 1) / 2) / (param.sample_pixel);
    ...
    for (int index = 0; index < param.angle_nums; index++) {
        ...
        memcpy(angle_im, &image_orientation[index * param.image_pixel],
               param.image_pixel * sizeof(double));
        SparseFilter2(angle_im, kernel, param.patch_size, angle_conv_im);
        memcpy(&conv_im[index * param.image_pixel], angle_conv_im,
               param.image_pixel * sizeof(double));
    }
...
void SIFT::CalcSIFT(BYTE* gray_im, double* sift_feature) {
    ...
    for (int pt = 0; pt < param.image_pixel; pt++) {
        ...
        if (lf_gray_im[pt] > max) max = lf_gray_im[pt];
    }
    ...
    for (int location_x = param.patch_size / 2;
         location_x <= param.image_height - (param.patch_size / 2);
         location_x += param.grid_spacing) {
        for (int location_y = param.patch_size / 2;
             location_y <= param.image_width - (param.patch_size / 2);
             location_y += param.grid_spacing) {
            memset(patch_feature, 0, param.patch_dims * sizeof(double));

            double l2_norm = 0.000001;
            int Point_cnt = 0;

            for (int p_x = -param.patch_size / 2;
                 p_x <= param.patch_size / 2 - param.sample_pixel;
                 p_x += param.sample_pixel) {
                for (int p_y = -param.patch_size / 2;
                     p_y <= param.patch_size / 2 - param.sample_pixel;
                     p_y += param.sample_pixel) {
                    int i = location_x + p_x;
                    int j = location_y + p_y;

                    for (int index = 0; index < param.angle_nums; index++) {
                        patch_feature[Point_cnt] =
                            conv_im[index * param.image_pixel +
                                    j * param.image_height + i];
            ...
            memcpy(&sift_feature[patch_cnt * param.patch_dims], patch_feature,
                   param.patch_dims * sizeof(double));
            patch_cnt += 1;
        }
    }
...
void SIFT::ImageOrientation(double* gray_im, double* image_orientation) {
    ...
    for (int index = 0; index < param.angle_nums; index++) {
        for (int pt = 0; pt < param.image_pixel; pt++) {
            double tmp1 = im_cos_theta[pt] * cos_array[index] +
                          im_sin_theta[pt] * sin_array[index];
            ...
            if (tmp > 0)
                ...
                tmp = 0;
            image_orientation[index * param.image_pixel + pt] =
                tmp * im_magnitude[pt];
        }
    }
