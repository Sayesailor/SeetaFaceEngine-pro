...
class CCFAN {
   public:
    ...
    /** Initialize the facial landmark detection model.
      *  @param model_path Path of the model file, either absolute or relative
     * to
      *                   the working directory.
      */
    void InitModel(const char *model_path);

    /** Detect five facial landmarks, i.e., two eye centers, nose tip and two
     * mouth corners.
      ...
      */
    void FacialPointLocate(const unsigned char *gray_im, int im_width,
   ...
   private:
    /** Extract shape indexed SIFT features.
      ...
      *  @param patch_size The size of the patch used for extracting SIFT
     * feature
      *  @param[out] sift_fea the extracted shape indexed SIFT features which
     * are concatenated into a vector
      */
    void TtSift(const unsigned char *gray_im, int im_width, int im_height,
                float *face_shape, int patch_size, double *sift_fea);

    /** Extract a image patch which is centered at point(point_x, point_y) with
    * a given patch size.
    *  @param gray_im A grayscale image
    *  @param im_width The width of the inpute image
    *  @param im_height The height of the inpute image
    ...
    */
    void GetSubImg(const unsigned char *gray_im, int im_width, int im_height,
                   float point_x, float point_y, int patch_size, BYTE *sub_img);
