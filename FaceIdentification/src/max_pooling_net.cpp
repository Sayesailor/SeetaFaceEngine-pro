 ...
 *China.
 *
 * The codes are mainly developed by Zining Xu(a M.S. supervised by Prof.
 *Shiguang Shan)
 *
...
void MaxPoolingNet::SetUp() {
    kernel_h_ = kernel_w_ = *(int*)(this->hyper_param()->param("kernel_size"));
    stride_h_ = stride_w_ = *(int*)(this->hyper_param()->param("stride"));
...
void MaxPoolingNet::Execute() {
    ...
    int dst_h = static_cast<int>(
                    ceil(static_cast<float>(src_h - kernel_h_) / stride_h_)) +
                1;
    int dst_w = static_cast<int>(
                    ceil(static_cast<float>(src_w - kernel_w_) / stride_w_)) +
                1;
