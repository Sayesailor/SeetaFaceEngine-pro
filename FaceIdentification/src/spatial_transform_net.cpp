    ...
    for (int n = 0; n < num; ++n) {
        ...
        for (int x = 0; x < dst_h; ++x)
            for (int y = 0; y < dst_w; ++y) {
                ...
                // feature map.
                double src_y =
                    theta_data[0] * y + theta_data[1] * x + theta_data[2];
                double src_x =
                    theta_data[3] * y + theta_data[4] * x + theta_data[5];
                for (int c = 0; c < channels; ++c) {
                    if (!is_mat_data_) {
                        output_data[output->offset(n, c, x, y)] =
                            Sampling(input_data + input->offset(n, c), src_h,
                                     src_w, src_x, src_y, 1.0 / scale);
                    } else {
                        output_data[output->offset(n, c, x, y)] = Sampling(
                            reinterpret_cast<unsigned char*>(input_data +
                                                             input->offset(n)),
                            c, src_h, src_w, this->output_blobs(0)->channels(),
...
double SpatialTransformNet::Sampling(const float* const feat_map, int H, int W,
    ...
    if (type_ == "linear") {
        ...
        if (ux >= 0 && ux < H - 1 && uy >= 0 && uy < W - 1) {
            ...
            ans = (1 - cof_x) * ans +
                  cof_x * ((1 - cof_y) * feat_map[offset + W] +
        ...
        if (x >= 0 && x < H && y >= 0 && y < W) {
            ...
            double kernel_width =
                std::max(8.0, 4.0 / scale);  // bicubic kernel width
...
double SpatialTransformNet::Sampling(const unsigned char* const feat_map, int c,
                                     int H, int W, int C, double x, double y,
                                     double scale) {
    if (type_ == "linear") {
        // bilinear subsampling
        int ux = floor(x), uy = floor(y);
        ...
        if (ux >= 0 && ux < H - 1 && uy >= 0 && uy < W - 1) {
            ...
            ans = (1 - cof_x) * ans +
                  cof_x * ((1 - cof_y) * feat_map[offset + W * C] +
        ...
        if (x >= 0 && x < H && y >= 0 && y < W) {
            ...
            for (int i = 0; i < lx; ++i) {
                if (i == 0 || indices_x[i] != indices_x[i - 1]) {
                    ...
                    for (int j = 0; j < ly; ++j) {
                        val += feat_map[offset + indices_y[j] * C + c] *
                               weights_y[j];
                    }
                }
                ans += val * weights_x[i];
