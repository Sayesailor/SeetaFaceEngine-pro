...
void Blob::Permute(int dim1, int dim2, int dim3, int dim4) {
        ...
        for (idx[1] = 0; idx[1] < shape_[dim[1]]; ++idx[1]) {
            for (idx[2] = 0; idx[2] < shape_[dim[2]]; ++idx[2]) {
                for (idx[3] = 0; idx[3] < shape_[dim[3]]; ++idx[3]) {
                    tmp[cnt] = dat[offset(idx[redim[0]], idx[redim[1]],
                                          idx[redim[2]], idx[redim[3]])];
                    cnt++;
                }
            }
        }
    ...
    for (int i = 0; i < 4; ++i) tmp_shape[i] = shape_[dim[i]];
    for (int i = 0; i < 4; ++i) shape_[i] = tmp_shape[i];
    memcpy(dat, tmp, sizeof(float) * count_);
    delete[] tmp;
}

void Blob::Release() { data_ = nullptr; }

void Blob::SetData() {
    if (!data_) data_.reset(new float[count_], std::default_delete<float[]>());
}
...
void Blob::CopyData(int n, int c, int h, int w,
    ...
    for (int i = 0; i < count_; ++i) data_head[i] = data[i];
}
