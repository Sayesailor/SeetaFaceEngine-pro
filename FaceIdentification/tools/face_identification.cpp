...
FaceIdentification::~FaceIdentification() { delete recognizer; }
...
uint32_t FaceIdentification::crop_width() { return recognizer->crop_width(); }

uint32_t FaceIdentification::crop_height() { return recognizer->crop_height(); }
...
uint8_t FaceIdentification::ExtractFeatureWithCrop(
    const ImageData& src_image, const FacialLandmark* llpoint,
    FaceFeatures const feats) {
    float point_data[10];
    for (int i = 0; i < 5; ++i) {
        point_data[i * 2] = llpoint[i].x;
...
float FaceIdentification::CalcSimilarity(FaceFeatures const fc1,
                                         FaceFeatures const fc2, long dim) {
    if (dim == -1) {
        dim = recognizer->feature_size();
    }
    return simd_dot(fc1, fc2, dim) /
           (sqrt(simd_dot(fc1, fc1, dim)) * sqrt(simd_dot(fc2, fc2, dim)));
}
}
