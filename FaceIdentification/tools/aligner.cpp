...
#include <vector>
namespace seeta {
Aligner::Aligner() : crop_height_(256), crop_width_(256) {
    ...
    float std_points[10] = {
        89.3095,  72.9025, 169.3095, 72.9025,  127.8949,
        127.0441, 96.8796, 184.8907, 159.1065, 184.7601,
    };
...
void Aligner::Alignment(const ImageData& src_img, const float* const points,
                        Blob* const dst_blob) {
    ...
    memcpy(input_data->data().get(), src_img.data,
           input_data->count() * sizeof(unsigned char));
