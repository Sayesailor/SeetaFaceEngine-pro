...
class FaceIdentification::Recognizer {
   public:
    ...
    uint8_t ExtractFeature(unsigned char* const u_data, float* const feat,
                           int n = 1) {
        net_->input_blobs(0)
            ->CopyData(n, crop_height_, crop_width_, crop_channels_, u_data);
    ...
    uint32_t crop_mem_size() {
        return crop_width_ * crop_height_ * crop_channels_;
    }
   ...
   private:
    ...
    uint32_t feat_size_;
    uint8_t isLoadModel() { return net_ != nullptr; }
};
}
#endif  // RECOGNIZER_H_
