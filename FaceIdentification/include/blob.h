...
class Blob {
   public:
    ...
    inline int offset(const int n, const int c = 0, const int h = 0,
                      const int w = 0) const {
        if (n < 0 || n >= num() || c < 0 || c >= channels() || h < 0 ||
            h >= height() || w < 0 || w >= width()) {
            LOG(ERROR) << "Index: (" << n << ", " << c << ", " << h << ", " << w
                       << "), "
                       << "Bound: [" << num() << ", " << channels() << ", "
                       << height() << ", " << width() << "].";
            exit(0);
    ...
    inline const float operator[](int i) const { return data_.get()[i]; }
    inline float& operator[](int i) { return data_.get()[i]; }
