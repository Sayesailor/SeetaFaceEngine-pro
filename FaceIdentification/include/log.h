...
#define LOG(OPTION)                                                   \
    ViplLog(OPTION) << "[" << __FILENAME__ << ":" << __LINE__ << "] " \
                    << _END_COLOR_
...
#define CHECK_LE(A, B)                                                       \
    ...
    LOG(ERROR) << "Check failed:"                                            \
               << "(" << #A << " <= " << #B << ") "                          \
               << "(" << A << " vs. " << B << ") "                           \
               << "Inputs " << #A << " must be less than or equal to " << #B \
               << "."
...
#define CHECK_GE(A, B)                                                        \
    ...
    LOG(ERROR) << "Check failed:"                                             \
               << "(" << #A << " >= " << #B << ") "                           \
               << "(" << A << " vs. " << B << ") "                            \
               << "Inputs " << #A << " must be great than or equal to " << #B \
               << "."

// check int A is near to int B
#define CHECK_INT_NEAR(A, B, C)                                     \
    if (abs(A - B) > C)                                             \
    LOG(ERROR) << "Check failed:" << #A << " is not near to " << #B \
               << " within the range " << #C << "."

// check double A is neart to double B
#define CHECK_DOUBLE_NEAR(A, B, C)                                   \
    if (fabs(A - B) > C)                                             \
    LOG(ERROR) << "Check failed: " << #A << " is not near to " << #B \
               << " within the range " << #C << "."
