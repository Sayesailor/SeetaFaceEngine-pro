 ...
 *
 * This file is part of the SeetaFace Identification module, containing codes
 *implementing the
 ...
 *China.
 *
 * The codes are mainly developed by Wanglong Wu(a Ph.D supervised by Prof.
 *Shiguang Shan)
 *
...
class Net {
   public:
    ...
    Net* const father() { return father_; }
    void SetFather(Net* father) { father_ = father; }
    std::vector<std::shared_ptr<Net> >& nets() { return nets_; }
    std::shared_ptr<Net> nets(int i) { return nets_[i]; }
    std::vector<Blob>& input_blobs() { return input_blobs_; }
    Blob* input_blobs(int i) { return &(input_blobs_[i]); }
    std::vector<Blob>& output_blobs() { return output_blobs_; }
    Blob* output_blobs(int i) { return &(output_blobs_[i]); }
    std::vector<std::vector<Blob*> >& output_plugs() { return output_plugs_; }
    std::vector<Blob*>& output_plugs(int i) { return output_plugs_[i]; }
    std::vector<std::vector<Blob*> >& input_plugs() { return input_plugs_; }
    std::vector<Blob*>& input_plugs(int i) { return input_plugs_[i]; }
    HyperParam* hyper_param() { return &hyper_params_; }
    std::vector<Blob>& params() { return params_; }
    Blob* params(int i) { return &(params_[i]); }
